...
class View:
    ...
    def _add_operations_keypad(self, frame):
        ...
        self.dot = Tk.Button(frame, text=".")
        self.dot.grid(row=4, column=5)
...
class Controller:
    ...
    def __init__(self, my_model, my_view):
        ...
        self.view.dot.bind("<Button>", lambda event, op='.': self.operation_pad_callback(op))
    ...
    def keystroke_callback(self, event):
        ...
        if event.keysym in '0123456789':
            ...
            print('\n >>> The keystroke "{}" is not a number or operation. <<<' .format(event.keysym))
            return
